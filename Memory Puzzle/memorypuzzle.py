...
WINDOWSWIDTH = 640 # size of window's width
WINDOWSHEIGHT = 480 # size of window's height
REVEALSPEED = 8 # speed boxes' sliding reveals and covers
BOXSIZE = 50 # size of box height and width in pixels
GAPSIZE = 10 # size between boxes in pixels.
BOARDWIDTH = 4 # number of columns of icons
BOARDHEIGHT = 3 # number of rows of icons
assert (BOARDHEIGHT * BOARDWIDTH) % 2 == 0, 'Board needs to have an even number' \
                                            ' of boxes for pairs of matches.'
...
def main():
    ...
    mousex = 0 # var stores x coordinate of mouse event
    mousey = 0 # var stores y coordinate of mouse event
            ...
            if not revealedBoxes[boxx][boxy] and mouseClicked:
                ...
                if firstSelection == None:
                    ...
                    if icon1shape != icon2shape or icon1color != icon2color:
                        ...
                        startGameAnimation(mainBoard)
                    firstSelection = None # reset the first selection var
...
def splitIntoGroupsOf(groupSize, theList):
    # splits a list into a lists of lists, where the inner lists have at
...
def revealBoxesAnimation(board, boxesToReveal):
    # does the box reveal animation
    for coverage in range(BOXSIZE, (-REVEALSPEED) - 1, - REVEALSPEED):
        drawBoxCovers(board, boxesToReveal, coverage)


def coverBoxesAnimation(board, boxesToCover):
    # does the box cover animation
    for coverage in range(0, BOXSIZE + REVEALSPEED, REVEALSPEED):
        drawBoxCovers(board, boxesToCover, coverage)

def drawBoard(board, revealed):
    # draws boxes in cover or revealed state
    for boxx in range(BOARDWIDTH):
        for boxy in range(BOARDHEIGHT):
            left, top = leftTopCoordsOfBox(boxx, boxy)
...
def gameWonAnimation(board):
    # flashes blackgroud color when game has been won.
...
def hasWon(revealedBoxes):
    # rturns true if all the boxes have been revealed, otherwise false.
